...
function hasItem(item, count)
    invitems = API.InvItemcount_String(item)
...
function getEnemies(names, size)
    --returns table of enemy names
    --size = true return # of enemies
    local NPCs = {}
    if names then
...
function findEnemyNearLeader()
    ...
    for i = 1, #NPCs do
        ...
        if currentTarget ~= ENEMIES.Elite_Sotapanna then
            lifeToCheck = 100000
        end
...
function prayerCheck()
    if PRAYER_TO_USE == nil then
        if PROTECT_MAGIC.BUFF_ID and hasBuff(PROTECT_MAGIC.BUFF_ID) then
            activateAbility(PROTECT_MAGIC.SPELL_NAME)
            API.RandomSleep2(800, 50, 300)    
        elseif PROTECT_MELEE.BUFF_ID and hasBuff(PROTECT_MELEE.BUFF_ID) then
            activateAbility(PROTECT_MELEE.SPELL_NAME)
            API.RandomSleep2(800, 50, 300)
        elseif PROTECT_RANGED.BUFF_ID and hasBuff(PROTECT_RANGED.BUFF_ID) then
            activateAbility(PROTECT_RANGED.SPELL_NAME)
            API.RandomSleep2(800, 50, 300)
        elseif PROTECT_NECRO.BUFF_ID and hasBuff(PROTECT_NECRO.BUFF_ID) then
            activateAbility(PROTECT_NECRO.SPELL_NAME)
            API.RandomSleep2(800, 50, 300)
        elseif SOUL_SPLIT.BUFF_ID and hasBuff(SOUL_SPLIT.BUFF_ID) then
...
function waitForParty()
    ...
    while not PartyInRange(5) do
        ...
        if API.GetLocalPlayerName() == p1 or API.GetLocalPlayerName() == p2 then
            local leader = findLeader(true)
            if leader and leader.Distance > 0 then
...
function enterDungeon()
    ...
    if API.GetLocalPlayerName() == Leader_Name then
        ...
        API.RandomSleep2(600, 0, 600) -- brief sync
        API.DoAction_Object1(0x39,API.OFF_ACT_GeneralObject_route0,{ templeDoorID },50)
        API.RandomSleep2(600, 0, 600)
    end   
end
...
function exitDungeon()
    ...
    local doorObj
    while true do
        local doorList = API.ReadAllObjectsArray({12}, {doorID}, {})
        doorObj = (#doorList > 0) and doorList[1] or nil
        if doorObj and doorObj.Distance <= 1 then break end
        if doorObj and doorObj.Distance < 10 and PartyInRange(4) then break end
        antiban()
        API.RandomSleep2(600, 0, 250)
    end
    API.DoAction_Object1(0x39,API.OFF_ACT_GeneralObject_route0,{ doorID },50)
    while inOrOut() ~= "outside" do
        API.RandomSleep2(600, 0, 250)
    end  
end
function doDungeon()
...
do-----------------------------------------------------------------------------------
    ...
    local debug = {
        ...
        {"Prayer_To_Use:", (PRAYER_TO_USE and PRAYER_TO_USE.SPELL_NAME) or "none"},
        {"Current_Target:", tostring(currentTarget or "none")}
        }
        API.DrawTable(debug)
    ----DEBUG----
   doDungeon() 
